exports.run = {
  ...
  async: async (m, { client, args, isPrefix, command, Func, env }) => {
    ...
    if (command == "prefix") {
      ...
      if (env.evaluate_chars.includes(args[0])) {
        return client.reply(
          m.chat,
          Func.texted("bold", `🚩 Cannot use prefix ${args[0]} because it will cause an error.`),
          m,
        )
      }
      if (args[0] == system.prefix) {
        return client.reply(m.chat, Func.texted("bold", `🚩 Prefix ${args[0]} is currently used`), m)
      }
      ...
      if (env.evaluate_chars.includes(args[0])) {
        return client.reply(
          m.chat,
          Func.texted("bold", `🚩 Cannot add prefix ${args[0]} because an error will occur.`),
          m,
        )
      }
      if (system.prefix.includes(args[0])) {
        return client.reply(m.chat, Func.texted("bold", `🚩 Prefix ${args[0]} already exists in the database.`), m)
      }
      ...
      if (!args || !args[0]) return client.reply(m.chat, Func.example(isPrefix, command, "#"), m)
      // if (args[0].length > 1) return client.reply(m.chat, Func.texted('bold', `🚩 Enter only 1 prefix.`), m)
      if (system.prefix.length < 2) {
        return client.reply(m.chat, Func.texted("bold", `🚩 Can't removing more prefix.`), m)
      }
      if (!system.prefix.includes(args[0])) {
        return client.reply(m.chat, Func.texted("bold", `🚩 Prefix ${args[0]} not exists in the database.`), m)
      }
